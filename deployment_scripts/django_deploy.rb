...
# Make stdout synchronous
STDOUT.sync = true

### This script automates the 2nd part of django deployment, extracting the package and restarting apache.
# Exit if not root
Kernel.abort("Must run a root") unless Process.uid == 0
PKG_TAR_GZ = ARGV[0]
APP_NAME   = ARGV[1]
Kernel.abort("Invalid arguments! tarball and app name required.") unless (PKG_TAR_GZ or APP_NAME)
...
[
  ...
  "#{PYTHON_ENV} hisquare/manage.py migrate",
  start_apache,
